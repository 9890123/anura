...
#else
#pragma comment(linder, "/SUBSYSTEM:CONSOLE")
#endif
#endif
...
namespace
{
	...
	PREF_BOOL(resizable, false, "Window is dynamically resizeable.");
	...
	void print_help(const std::string& argv0)
	{
		std::cout << "Usage: " << argv0 << "[OPTIONS]\n" <<
			...
			"      --native                 one picel in-game equals one pixel on monitor\n" <<
			"      --relay                  use the server as relay in multiplayer rather\n" <<
			...
			"      --width[=]NUM            sets the game window width to which contents are\m" <<
			"                                 scaled\n" <<
...
int main(int argcount, char* argvec[])
{
	...
	if (g_auto_update_module || g_auto_update_anura != "") {
		...
		ffl::InstrusivePtr<module::client> cl, anura_cl;
			...
			while (static_cast<int>(profile::get_tick_time()) < target_end && (cl || anura_cl)) {
				if (cl && !cl->process()) {
					if (cl->error().empty() == false) {
						...
						update_require_restart = cl->files_written() != 0;
					}
					cl.reset();
				}
	...
	const state::Manager stats_manager;
	...
	try {
		...
		loader.setNumberOfItems(preload_items++);
		CustomObject::init();
